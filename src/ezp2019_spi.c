...
static int ezp_reset_and_reinit(void) {
  int ret;

  /* Sending a USB-level reset forces the device to re-enumerate and
   * clears any stale chip state cached in the programmer firmware. */
  ret = libusb_reset_device(ezp_handle);
  if (ret != 0)
    return -1;

  /* Wait for the device to settle after re-enumeration, then detach
   * any kernel driver that may have re-attached and re-claim. */
  usleep(200000);
  libusb_set_auto_detach_kernel_driver(ezp_handle, 1);
  if (libusb_kernel_driver_active(ezp_handle, 0) == 1)
    libusb_detach_kernel_driver(ezp_handle, 0);

  /* Some EZP models need to read USB string descriptors before they
   * will accept commands on the bulk endpoints — use this as warm-up. */
  {
    uint8_t tmp[256];
  ...
  if (ret != 0) {
    EZP_DEBUG("ezp_reset_and_reinit: claim interface failed: %s\n",
              libusb_error_name(ret));
    return -1;
  }
...
static int ezp_do_connect(void) {
  ...
  EZP_DEBUG("ezp_do_connect: probing chip\n");

  prepare_command_packet(packet, EZP_CMD_CONNECT, 0, 256, 0, 1000, 0);
