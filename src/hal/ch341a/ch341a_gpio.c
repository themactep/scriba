...
static int usb_transf(const char *func, uint8_t type, uint8_t *buf, int len) {
  ...
  ret =
      libusb_bulk_transfer(handle, type, buf, len, &actuallen, DEFAULT_TIMEOUT);
  if (ret < 0) {
    ...
    if (debug_enabled)
      fprintf(stderr,
              "[DEBUG] %s: libusb_bulk_transfer failed with error %d (%s)\n",
...
int ch341a_gpio_setdir(void) {
  if (debug_enabled)
    fprintf(
        stderr,
        "[DEBUG] ch341a_gpio_setdir: setting GPIO direction (mask=0x%02X)\n",
        DIR_MASK);

  uint8_t buf[] = {CH341A_CMD_UIO_STREAM, CH341A_CMD_UIO_STM_DIR | DIR_MASK,
...
int ch341a_gpio_setbits(uint8_t bits) {
  if (debug_enabled)
    fprintf(stderr,
            "[DEBUG] ch341a_gpio_setbits: setting GPIO bits to 0x%02X\n", bits);

  uint8_t buf[] = {CH341A_CMD_UIO_STREAM, CH341A_CMD_UIO_STM_OUT | bits,
                   CH341A_CMD_UIO_STM_END};
...
int ch341a_gpio_getbits(uint8_t *data) {
  ...
  uint8_t buf[] = {CH341A_CMD_UIO_STREAM, CH341A_CMD_UIO_STM_IN,
                   CH341A_CMD_UIO_STM_END};
