...
void ch341ReadCmdMarshall(uint8_t *buffer, uint32_t addr,
  ...
  *ptr++ =
      ...
      ((*eeprom_info).addr_size + 1); // 2: I2C bus adddress + EEPROM address
  ...
  *ptr++ = ((EEPROM_I2C_BUS_ADDRESS | msb_addr) << 1) | 1; // 8/7: Read command
  ...
  static const unsigned char i2c_cfg_tail[] = {0x00, 0x06, 0x04, 0x00, 0x00,
                                               0x00, 0x00, 0x00, 0x00};
  static const unsigned char i2c_cfg_mid[] = {0x00, 0x00, 0x11, 0x4d, 0x40,
                                              0x77, 0xcd, 0xab, 0xba, 0xdc};
  static const unsigned char i2c_frame2[] = {
      0xe0, 0x00, 0x00, 0xc4, 0xf1, 0x12, 0x00, 0x11, 0x4d, 0x40, 0x77,
      0xf0, 0xf1, 0x12, 0x00, 0xd9, 0x8b, 0x41, 0x7e, 0x00, 0xe0, 0xfd,
      0x7f, 0xf0, 0xf1, 0x12, 0x00, 0x5a, 0x88, 0x41, 0x7e};
  static const unsigned char i2c_frame3[] = {
      0xe0, 0x00, 0x00, 0x2a, 0x88, 0x41, 0x7e, 0x06, 0x04, 0x00, 0x00,
      0x11, 0x4d, 0x40, 0x77, 0xe8, 0xf3, 0x12, 0x00, 0x14, 0x00, 0x00,
...
int32_t ch341readEEPROM(uint8_t *buffer, uint32_t bytestoread,
                        struct EEPROM *eeprom_info) {
  ...
  if (!xferBulkIn || !xferBulkOut) {
    fprintf(stderr,
            "Couldn't allocate USB transfer structures\n"); // Use stderr
    if (debug_enabled)
  ...
  libusb_fill_bulk_transfer(xferBulkIn, handle, BULK_READ_ENDPOINT,
                            ch341inBuffer, EEPROM_READ_BULKIN_BUF_SZ, cbBulkIn,
                            NULL, DEFAULT_TIMEOUT);

  libusb_fill_bulk_transfer(xferBulkOut, handle, BULK_WRITE_ENDPOINT,
                            ch341outBuffer, EEPROM_READ_BULKOUT_BUF_SZ,
  ...
  while (1) {
    ...
    if (ret < 0 || (int32_t)getnextpkt == -1) { // indicates an error
      ...
      if (ret < 0) {
        ...
        if (debug_enabled)
          fprintf(stderr,
                  "[DEBUG] ch341readEEPROM: libusb_handle_events_timeout error "
                  "%d\n",
                  ret);
      }
      if (debug_enabled)
        fprintf(stderr,
                "[DEBUG] ch341readEEPROM: read operation failed, aborting\n");
      libusb_free_transfer(xferBulkIn);
      libusb_free_transfer(xferBulkOut);
      return -1;
    }
    if (getnextpkt ==
        1) {          // callback function reports a new BULK IN packet received
      ...
      libusb_submit_transfer(xferBulkIn); // re-submit request for next BULK IN
                                          // packet of EEPROM data
      if (syncackpkt)
        syncackpkt = 0;
      ...
      if (readpktcount == 4) {
        ...
        ch341ReadCmdMarshall(ch341outBuffer, byteoffset,
                             eeprom_info); // Fill output buffer
        libusb_fill_bulk_transfer(xferBulkOut, handle, BULK_WRITE_ENDPOINT,
                                  ch341outBuffer, EEPROM_READ_BULKOUT_BUF_SZ,
                                  cbBulkOut, NULL, DEFAULT_TIMEOUT);

        libusb_submit_transfer(
            xferBulkOut); // update transfer struct (with new EEPROM page
                          // offset) and re-submit next transfer request to BULK
                          // OUT endpoint
      }
    }
  }
...
void cbBulkOut(struct libusb_transfer *transfer __attribute__((unused))) {
  ...
  if (transfer->status != LIBUSB_TRANSFER_COMPLETED && debug_enabled)
    fprintf(stderr,
            "[DEBUG] cbBulkOut: transfer completed with non-success status %d "
            "(%s)\n",
            transfer->status, libusb_error_name(transfer->status));
  return;
}
...
int32_t ch341writeEEPROM(uint8_t *buffer __attribute__((unused)),
                         uint32_t bytesum,
                         struct EEPROM *eeprom_info __attribute__((unused))) {
