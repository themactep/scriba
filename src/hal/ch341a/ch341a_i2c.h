...
/* Based on (closed-source) DLL V1.9 for USB by WinChipHead (c) 2005.
   ...
   This can be a problem for copyright, sure asbokid can't release this part on
   any GPL licence*/

#define mCH341_PACKET_LENGTH                                                   \
  32                           /* wMaxPacketSize 0x0020  1x 32 bytes, Unused   \
                                */
#define mCH341_PKT_LEN_SHORT 8 /* wMaxPacketSize 0x0008  1x 8 bytes, Unused */

#define mCH341_ENDP_INTER_UP                                                   \
  0x81 /* bEndpointAddress 0x81  EP 1 IN (Interrupt), Unused */
#define mCH341_ENDP_INTER_DOWN                                                 \
  0x01 /* This endpoint is not listed in my lsusb -v output, Unused */
...
/* End of the part based on close-sourced DLL V1.9 for USB by WinChipHead (c)
   2005. Since it is largely unused we can replace it. */
...
#define CH341_EEPROM_READ_CMD_SZ                                               \
  0x65 /* Same size for all 24cXX read setup and next packets*/
...
int32_t ch341readEEPROM(uint8_t *buf, uint32_t bytes,
                        struct EEPROM *eeprom_info);
int32_t ch341writeEEPROM(uint8_t *buf, uint32_t bytes,
                         struct EEPROM *eeprom_info);
