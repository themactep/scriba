...
struct spi_backend {
  ...
  SPI_CONTROLLER_RTN_T (*write_read_nbyte)(u8 *ptr_data, u32 writelen,
                                           u8 *ptr_rtn_data, u32 readlen);
  SPI_CONTROLLER_RTN_T (*chip_select_low)(void);
  SPI_CONTROLLER_RTN_T (*chip_select_high)(void);
  void (*set_flash_params)(uint32_t size, uint16_t pagesize, uint16_t protocol);
};
...
static SPI_CONTROLLER_RTN_T ch_write_read_nbyte(u8 *ptr_data, u32 writelen,
                                                u8 *ptr_rtn_data, u32 readlen) {
  return (SPI_CONTROLLER_RTN_T)ch341a_spi_send_command(writelen, readlen,
                                                       ptr_data, ptr_rtn_data);
}
...
static void ch_set_flash_params(uint32_t size, uint16_t pagesize,
                                uint16_t protocol) {
  (void)size;
  (void)pagesize;
  (void)protocol;
...
static SPI_CONTROLLER_RTN_T ezp_write_read_nbyte(u8 *ptr_data, u32 writelen,
                                                 u8 *ptr_rtn_data, u32 readlen) {
  return (SPI_CONTROLLER_RTN_T)ezp2019_spi_send_command(writelen, readlen,
                                                        ptr_data, ptr_rtn_data);
}
...
static void ezp_set_flash_params(uint32_t size, uint16_t pagesize,
                                 uint16_t protocol) {
  ezp_set_chip_params(size, pagesize, protocol);
}
...
static const struct spi_backend ch341a_backend = {
    ...
    .write_read_nbyte = ch_write_read_nbyte,
    .chip_select_low = ch_chip_select_low,
    .chip_select_high = ch_chip_select_high,
    .set_flash_params = ch_set_flash_params,
};
...
static const struct spi_backend ezp2019_backend = {
    ...
    .write_read_nbyte = ezp_write_read_nbyte,
    .chip_select_low = ezp_chip_select_low,
    .chip_select_high = ezp_chip_select_high,
    .set_flash_params = ezp_set_flash_params,
};
...
/**
 * Write N bytes then read N bytes in one SPI transaction
 * @param ptr_data Buffer of data to write
 * @param writelen Number of bytes to write
 * @param ptr_rtn_data Buffer to receive read data
 * @param readlen Number of bytes to read
 * @return SPI_CONTROLLER_RTN_NO_ERROR on success
 */
SPI_CONTROLLER_RTN_T SPI_CONTROLLER_WriteRead_NByte(u8 *ptr_data, u32 writelen,
                                                     u8 *ptr_rtn_data,
                                                     u32 readlen) {
  return (SPI_CONTROLLER_RTN_T)active_backend->write_read_nbyte(
      ptr_data, writelen, ptr_rtn_data, readlen);
}
...
void SPI_CONTROLLER_Set_Flash_Params(uint32_t size, uint16_t pagesize,
                                     uint16_t protocol) {
  if (active_backend->set_flash_params)
    active_backend->set_flash_params(size, pagesize, protocol);
}
