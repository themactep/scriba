...
int mw_eeprom_erase(unsigned long offs, unsigned long len) {
  ...
  if (offs || len < (unsigned long)mw_eepromsize) {
    if (Write_EEPROM_3wire(pbuf, mw_eepromsize) < 0) {
      fprintf(stderr,
              "Failed to erase [%lu] bytes of [%s] EEPROM address 0x%08lu\n",
...
int mw_eeprom_write(unsigned char *buf, unsigned long to, unsigned long len) {
  ...
  if (Write_EEPROM_3wire(pbuf, mw_eepromsize) < 0) {
    fprintf(stderr,
            "Failed to write [%lu] bytes of [%s] EEPROM address 0x%08lu\n", len,
            eepromname, to); // Use stderr
    return -1;
  }
...
long mw_init(void) {
  ...
  if (fix_addr_len) {
    printf("Microwire EEPROM chip: %s, Size: %d bytes, Org: %d bits, fix addr "
           "len: %d\n",
...
void support_mw_eeprom_list(void) {
  ...
  for (i = 0;
       (unsigned long)i < (sizeof(mw_eepromlist) / sizeof(struct MW_EEPROM));
       i++) {
