...
typedef enum {
  ...
  SPI_NAND_FLASH_RTN_SPI_CTRL_FAIL, // Added for SPI controller communication
                                    // errors
 ...
 * @param status Pointer to store the operation status (e.g., bad block
 * detection).
...
/**
 * @brief Enable or disable the on-die ECC feature based on global ECC_fcheck
 * flag.
 * @return SPI_NAND_FLASH_RTN_NO_ERROR on success.
 */
