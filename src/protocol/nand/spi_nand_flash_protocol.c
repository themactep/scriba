...
spi_nand_protocol_get_status_reg_1_cached(u8 *ptr_rtn_protection);
static SPI_NAND_FLASH_RTN_T
spi_nand_protocol_get_status_reg_2_cached(u8 *ptr_rtn_feature);
static SPI_NAND_FLASH_RTN_T
spi_nand_protocol_get_status_reg_3_cached(u8 *ptr_rtn_status);
...
SPI_NAND_FLASH_RTN_T spi_nand_protocol_get_feature(u8 addr, u8 *ptr_rtn_data) {
  ...
  if (spi_ret != SPI_CONTROLLER_RTN_NO_ERROR)
    goto spi_fail;
  spi_ret = _SPI_NAND_READ_NBYTE(ptr_rtn_data, _SPI_NAND_LEN_ONE_BYTE,
                                 SPI_CONTROLLER_SPEED_SINGLE);
  if (spi_ret != SPI_CONTROLLER_RTN_NO_ERROR)
    goto spi_fail;
  _SPI_NAND_READ_CHIP_SELECT_HIGH();
  return SPI_NAND_FLASH_RTN_NO_ERROR;
...
spi_nand_protocol_get_status_reg_1_cached(u8 *ptr_rtn_protection) {
  ...
  return spi_nand_protocol_get_feature(_SPI_NAND_ADDR_PROTECTION,
                                       ptr_rtn_protection);
}
...
SPI_NAND_FLASH_RTN_T spi_nand_protocol_get_status_reg_4(u8 *ptr_rtn_status) {
  return spi_nand_protocol_get_feature(_SPI_NAND_ADDR_FEATURE_4,
                                       ptr_rtn_status);
}
...
SPI_NAND_FLASH_RTN_T spi_nand_protocol_write_enable(void) {
  ...
  _SPI_NAND_READ_CHIP_SELECT_HIGH();
  return (spi_ret == SPI_CONTROLLER_RTN_NO_ERROR)
             ? SPI_NAND_FLASH_RTN_NO_ERROR
             : SPI_NAND_FLASH_RTN_SPI_CTRL_FAIL;
}
...
spi_nand_protocol_read_id(struct SPI_NAND_FLASH_INFO_T *ptr_rtn_flash_id) {
  ...
  spi_ret =
      _SPI_NAND_READ_NBYTE(&(ptr_rtn_flash_id->mfr_id), _SPI_NAND_LEN_ONE_BYTE,
                           SPI_CONTROLLER_SPEED_SINGLE);
  if (spi_ret != SPI_CONTROLLER_RTN_NO_ERROR)
    goto spi_fail;
  spi_ret =
      _SPI_NAND_READ_NBYTE(&(ptr_rtn_flash_id->dev_id), _SPI_NAND_LEN_ONE_BYTE,
                           SPI_CONTROLLER_SPEED_SINGLE);
  if (spi_ret != SPI_CONTROLLER_RTN_NO_ERROR)
    goto spi_fail;
  spi_ret =
      _SPI_NAND_READ_NBYTE(&(ptr_rtn_flash_id->dev_id_2),
                           _SPI_NAND_LEN_ONE_BYTE, SPI_CONTROLLER_SPEED_SINGLE);
  if (spi_ret != SPI_CONTROLLER_RTN_NO_ERROR)
    goto spi_fail;
...
spi_nand_protocol_read_id_3(struct SPI_NAND_FLASH_INFO_T *ptr_rtn_flash_id) {
  ...
  spi_ret = _SPI_NAND_READ_NBYTE(&dummy, _SPI_NAND_LEN_ONE_BYTE,
                                 SPI_CONTROLLER_SPEED_SINGLE);
  if (spi_ret != SPI_CONTROLLER_RTN_NO_ERROR)
    goto spi_fail;
  spi_ret =
...
spi_nand_protocol_read_from_cache(u32 data_offset, u32 len, u8 *ptr_rtn_buf,
  ...
  default:
    spi_ret =
        SPI_CONTROLLER_RTN_NO_ERROR; // Or perhaps an error for invalid mode?
    break;
  }
  if (spi_ret != SPI_CONTROLLER_RTN_NO_ERROR)
    goto spi_fail;
