...
// Returns 0 on success, -1 on SPI error
static int eeprom_write_byte(struct spi_eeprom *dev, uint32_t address,
                             uint8_t data) {
  uint8_t buf[5];
  SPI_CONTROLLER_RTN_T spi_ret;
  int ret = 0;
...
// Returns 0 on success, -1 on SPI error
static int eeprom_write_page(struct spi_eeprom *dev, uint32_t address,
                             int page_size, uint8_t *data) {
  ...
  SPI_CONTROLLER_Chip_Select_Low();
  spi_ret = SPI_CONTROLLER_Write_NByte(
      buf, offs + page_size); // Removed speed argument
  SPI_CONTROLLER_Chip_Select_High();
...
// Returns byte read on success, or -1 on SPI error (casting to uint8_t
// truncates) Caller must check for negative return value to detect error.
...
int spi_eeprom_read(unsigned char *buf, unsigned long from, unsigned long len) {
  ...
  for (i = 0; i < (uint32_t)seepromsize; i++) {
    ...
    if (timer_progress()) {
      printf("\bRead %d%% [%d] of [%d] bytes      ", 100 * i / seepromsize, i,
             seepromsize);
...
int spi_eeprom_erase(unsigned long offs, unsigned long len) {
  ...
  for (i = 0; i < (uint32_t)seepromsize; i++) {
    ...
    if (timer_progress()) {
      printf("\bErase %d%% [%d] of [%d] bytes      ", 100 * i / seepromsize, i,
             seepromsize);
      printf("\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
             "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b");
      fflush(stdout);
...
int spi_eeprom_write(unsigned char *buf, unsigned long to, unsigned long len) {
  ...
  for (i = 0; i < (uint32_t)seepromsize; i++) {
    ...
    if (timer_progress()) {
      printf("\bWritten %d%% [%d] of [%d] bytes      ", 100 * i / seepromsize,
             i, seepromsize);
      printf("\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
             "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b");
      fflush(stdout);
...
void support_spi_eeprom_list(void) {
  ...
  for (i = 0;
       (unsigned long)i < (sizeof(seepromlist) / sizeof(struct spi_eeprom));
       i++) {
