...
extern SPI_NAND_FLASH_RTN_T spi_nand_protocol_get_feature(u8 addr,
                                                          u8 *ptr_rtn_data);
extern SPI_NAND_FLASH_RTN_T spi_nand_protocol_set_status_reg_1(u8 protection);
extern SPI_NAND_FLASH_RTN_T
spi_nand_protocol_get_status_reg_1(u8 *ptr_rtn_protection);
extern SPI_NAND_FLASH_RTN_T spi_nand_protocol_set_status_reg_2(u8 feature);
extern SPI_NAND_FLASH_RTN_T
spi_nand_protocol_get_status_reg_2(u8 *ptr_rtn_feature);
extern SPI_NAND_FLASH_RTN_T
spi_nand_protocol_get_status_reg_3(u8 *ptr_rtn_status);
...
extern SPI_NAND_FLASH_RTN_T
spi_nand_protocol_program_load(u32 addr, u8 *ptr_data, u32 len, u32 write_mode);
...
spi_nand_flash_detect_device(struct SPI_NAND_FLASH_INFO_T *ptr_rtn_device_t) {
  SPI_NAND_FLASH_RTN_T rtn_status = SPI_NAND_FLASH_RTN_NO_ERROR;
  ...
  for (i = 0; spi_nand_flash_tables[i].ptr_name != NULL; i++) {
    if (flash_id.mfr_id == spi_nand_flash_tables[i].mfr_id) {
      if ((flash_id.dev_id == spi_nand_flash_tables[i].dev_id) &&
          ((flash_id.dev_id_2 == spi_nand_flash_tables[i].dev_id_2) ||
           (spi_nand_flash_tables[i].dev_id_2 == 0))) {
 ...
 */
SPI_NAND_FLASH_RTN_T
spi_nand_flash_init(struct SPI_NAND_FLASH_INFO_T *ptr_device_t) {
  SPI_NAND_FLASH_RTN_T rtn_status = SPI_NAND_FLASH_RTN_NO_ERROR;
  u8 feature;
 ...
 */
SPI_NAND_FLASH_RTN_T spi_nand_flash_read_page(u32 page_number,
                                              u32 column_number, u32 len,
                                              u8 *ptr_rtn_buf) {
  SPI_NAND_FLASH_RTN_T rtn_status = SPI_NAND_FLASH_RTN_NO_ERROR;
  ...
  if ((ptr_dev_info_t->feature & SPI_NAND_FLASH_DIE_SELECT_1_HAVE) &&
      (page_number >=
       (ptr_dev_info_t->device_size / 2) / ptr_dev_info_t->erase_size * 64)) {
    if (_die_id != 1) {
      _die_id = 1;
  ...
  rtn_status = spi_nand_protocol_read_from_cache(
      column_number, len, ptr_rtn_buf, ptr_dev_info_t->read_mode,
 ...
 */
SPI_NAND_FLASH_RTN_T spi_nand_flash_write_page(u32 page_number,
                                               u32 column_number, u32 len,
                                               u8 *ptr_buf) {
  SPI_NAND_FLASH_RTN_T rtn_status = SPI_NAND_FLASH_RTN_NO_ERROR;
  struct SPI_NAND_FLASH_INFO_T *ptr_dev_info_t = &_current_flash_info_t;
  ...
  /* Load data to cache */
  rtn_status = spi_nand_protocol_program_load(column_number, ptr_buf, len,
...
SPI_NAND_FLASH_RTN_T spi_nand_flash_erase_block(u32 block_idx) {
  ...
  page_number =
      block_idx * (ptr_dev_info_t->erase_size / ptr_dev_info_t->page_size);
  ...
  if ((ptr_dev_info_t->feature & SPI_NAND_FLASH_DIE_SELECT_1_HAVE) &&
    ...
    page_number -=
        (ptr_dev_info_t->device_size / 2) / ptr_dev_info_t->erase_size * 64;
    block_idx =
        page_number / (ptr_dev_info_t->erase_size / ptr_dev_info_t->page_size);
  } else {
    if (_die_id != 0) {
      _die_id = 0;
      rtn_status = spi_nand_protocol_die_select_1(_die_id);
 ...
 */
SPI_NAND_FLASH_RTN_T
spi_nand_flash_get_info(struct SPI_NAND_FLASH_INFO_T *ptr_rtn_info_t) {
...
SPI_NAND_FLASH_RTN_T spi_nand_flash_get_ecc_status(u8 *ptr_rtn_ecc_status) {
  ...
  *ptr_rtn_ecc_status = (status & _SPI_NAND_VAL_ECC_STATUS_MASK) >>
                        _SPI_NAND_VAL_ECC_STATUS_OFFSET;
  return SPI_NAND_FLASH_RTN_NO_ERROR;
}
 ...
 */
SPI_NAND_FLASH_RTN_T spi_nand_flash_is_bad_block(u32 block_idx,
                                                 u8 *ptr_rtn_is_bad) {
  SPI_NAND_FLASH_RTN_T rtn_status = SPI_NAND_FLASH_RTN_NO_ERROR;
  struct SPI_NAND_FLASH_INFO_T *ptr_dev_info_t = &_current_flash_info_t;
  u32 page_number;
  ...
  rtn_status = spi_nand_flash_read_page(page_number, ptr_dev_info_t->page_size,
                                        1, &spare_data);
  if (rtn_status != SPI_NAND_FLASH_RTN_NO_ERROR) {
    return rtn_status;
  }
...
SPI_NAND_FLASH_RTN_T spi_nand_flash_mark_bad_block(u32 block_idx) {
  ...
  rtn_status = spi_nand_flash_write_page(page_number, ptr_dev_info_t->page_size,
                                         1, &bad_mark);
  if (rtn_status != SPI_NAND_FLASH_RTN_NO_ERROR) {
    return rtn_status;
  }
