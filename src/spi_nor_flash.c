...
int snor_erase_sector(unsigned long offset) {
  ...
  SPI_CONTROLLER_Chip_Select_Low();
  SPI_CONTROLLER_Write_NByte(addr_buf, addr_len);
  if (SPI_CONTROLLER_Chip_Select_High()) {
    if (spi_chip_info->addr4b)
      snor_4byte_mode(0);
    snor_clear_progress();
    return -1;
...
int snor_read(unsigned char *buf, unsigned long from, unsigned long len) {
  ...
  // snor_dbg("%s: from:%x len:%x \n", __func__, from, len); // Commented out
  // missing function
  ...
  while (remain_len > 0) {
    ...
    /* combine opcode + address into one USB transfer */
    unsigned char addr_buf[5];
    int addr_len = 0;
    addr_buf[addr_len++] = OPCODE_READ;
    if (spi_chip_info->addr4b)
      addr_buf[addr_len++] = (physical_read_addr >> 24) & 0xff;
    addr_buf[addr_len++] = (physical_read_addr >> 16) & 0xff;
    addr_buf[addr_len++] = (physical_read_addr >> 8) & 0xff;
    addr_buf[addr_len++] = physical_read_addr & 0xff;
    ...
    if ((data_offset + remain_len) < spi_chip_info->sector_size) {
      ...
      while (chunk_remain > 0) {
        ...
        if (SPI_CONTROLLER_Read_NByte(&buf[len - remain_len + chunk_off],
                                      chunk_len)) {
#else
      ...
      while (chunk_off < to_read) {
        u32 chunk_len =
            (to_read - chunk_off) > 4096 ? 4096 : (to_read - chunk_off);
        if (SPI_CONTROLLER_Read_NByte(&buf[len - remain_len + chunk_off],
                                      chunk_len)) {
#else
      if (SPI_CONTROLLER_Read_NByte(&buf[len - remain_len],
                                    spi_chip_info->sector_size - data_offset)) {
#endif
          SPI_CONTROLLER_Chip_Select_High();
          if (spi_chip_info->addr4b)
            snor_4byte_mode(0);
