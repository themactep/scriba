...
const char *usb_hal_backend_version(void)
{
	...
	static char buf[18];
	snprintf(buf, sizeof(buf), "%d.%d.%d", version->major, version->minor, version->micro);
	return buf;
}
...
static int ch341_alloc_transfers(struct usb_hal *hal)
{
	...
	libusb_fill_bulk_transfer(hal->transfer_out, hal->handle, WRITE_EP, NULL, 0, cb_out, NULL, USB_TIMEOUT);
	for (int i = 0; i < USB_IN_TRANSFERS; i++)
		libusb_fill_bulk_transfer(hal->transfer_ins[i], hal->handle, READ_EP, NULL, 0, cb_in, NULL, USB_TIMEOUT);
	return 0;
}
